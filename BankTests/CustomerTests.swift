...
class CustomerTests: XCTestCase {
    ...
    func testListAccounts() {
        let customer: Customer = Customer(firstName: "Richard", lastName: "Richardson", email: "r@r.com", accounts: [Savings(identifier: 123, balance: 425.00),Checking(identifier: 321, balance: 225.24)])
        
        let test = customer.listAccounts(identify: "r@r.com")
        let expected = [Savings(identifier: 123, balance: 425.00),Checking(identifier: 321, balance: 225.24)]
        
        XCTAssertEqual(test!, expected)
    }
    
    func testListAccounts2() {
        let customer: Customer = Customer(firstName: "Tom", lastName: "Thompson", email: "t@t.com", accounts: [Checking(identifier: 456, balance: 500.00)])
        
        let test = customer.listAccounts(identify: "t@t.com")
        let expected = [Checking(identifier: 456, balance: 500.00)]
